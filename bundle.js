...
function startTroubleshooter() {
  ...
  var testSuite = new _TestSuite.TestSuite();
  //
  // if (audio) {
  ...
  if (video) {
    ...
    var bandwidthTest = new _defaultTests.VideoBandwidthTest({ iceConfig: iceConfig, mediaOptions: mediaOptions }, function (err, logs) {
      setProperties({
...
var TestSuite = function () {
  ...
  _createClass(TestSuite, [{
    ...
    key: 'runNextTest',
    value: function runNextTest(done, troubleshootingLog) {
      var _this = this;
      ...
      test.start().then(function () {
        ...
        logger.info('WebRTC Troubleshooting results', _this.get('troubleshootingLog'));
        test.running = false;
        test.destroy();
...
var VideoFrameChecker = function () {
  ...
  _createClass(VideoFrameChecker, [{
    ...
    value: function getCurrentImageData_() {
      ...
      context.drawImage(videoElement_, 0, 0, canvas_.width, canvas_.height);
      return context.getImageData(0, 0, canvas_.width, canvas_.height);
    }
  }, {
    key: 'checkVideoFrame_',
    value: function (_checkVideoFrame_) {
      function checkVideoFrame_() {
        return _checkVideoFrame_.apply(this, arguments);
      }

      checkVideoFrame_.toString = function () {
        return _checkVideoFrame_.toString();
      };

      return checkVideoFrame_;
    }(function () {
      if (!running_) {
        return;
      }
      ...
      if (frameComparator.calculate(previousFrame_, imageData.data) > identicalFrameSsimThreshold) {
...
VideoFrameChecker.prototype = {
  ...
  checkVideoFrame_: function (_checkVideoFrame_2) {
    function checkVideoFrame_() {
      return _checkVideoFrame_2.apply(this, arguments);
    }

    checkVideoFrame_.toString = function () {
      return _checkVideoFrame_2.toString();
    };
...
var AdvancedCameraTest = function (_Test) {
  ...
  function AdvancedCameraTest() {
    ...
    tests.push(new CameraResolutionTest([[160, 120], [320, 180], [320, 240], [640, 360], [640, 480], [768, 576], [1024, 576], [1280, 720], [1280, 768], [1280, 800], [1920, 1080], [1920, 1200], [3840, 2160], [4096, 2160]]));
    return _this;
  }
  ...
  _createClass(AdvancedCameraTest, [{
    ...
    key: 'runNextTest',
    value: function (_runNextTest) {
      function runNextTest() {
        return _runNextTest.apply(this, arguments);
      }

      runNextTest.toString = function () {
        return _runNextTest.toString();
      };

      return runNextTest;
    }(function () {
      var testNum = arguments.length <= 0 || arguments[0] === undefined ? 0 : arguments[0];

      if (testNum >= tests.length) {
        return defer.resolve(log);
      }

      tests[testNum].run(function (results) {
        log.push(results);
        runNextTest(++testNum);
      });
    })
...
var CameraResolutionTest = function () {
  ...
  _createClass(CameraResolutionTest, [{
    ...
    value: function testExpectations_(report) {
      ...
      if (!resolutionMatchesIndependentOfRotationOrCrop_(report.actualVideoWidth, report.actualVideoHeight, report.mandatoryWidth, report.mandatoryHeight)) {
...
},{"../TestSuite":2,"../VideoFrameChecker":3,"../WebrtcCall":4,"jQuery":12}],6:[function(require,module,exports){
'use strict';
  ...
  _createClass(AudioTest, [{
    ...
    value: function start() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        localMedia.start(options, function (err) {
          if (err) {
        ...
        localMedia.on('volumeChange', function () {
...
},{"../TestSuite":2}],7:[function(require,module,exports){
'use strict';
...
},{"../TestSuite":2}],8:[function(require,module,exports){
'use strict';
...
function _inherits(subClass, superClass) { if (typeof superClass !== "function" && superClass !== null) { throw new TypeError("Super expression must either be null or a function, not " + typeof superClass); } subClass.prototype = Object.create(superClass && superClass.prototype, { constructor: { value: subClass, enumerable: false, writable: true, configurable: true } }); if (superClass) Object.setPrototypeOf ? Object.setPrototypeOf(subClass, superClass) : subClass.__proto__ = superClass; } // Creates a loopback via relay candidates and tries to send as many packets
  ...
  _createClass(DataChannelThroughputTest, [{
    ...
    value: function start() {
      ...
      if (!options.iceServers.length) {
        ...
        call = new _WebrtcCall2.default(options);
        call.setIceCandidateFilter(_WebrtcCall2.default.isRelay);
    ...
    key: 'sendingStep',
    value: function (_sendingStep) {
      function sendingStep() {
        return _sendingStep.apply(this, arguments);
      }

      sendingStep.toString = function () {
        return _sendingStep.toString();
      };
    ...
    value: function onMessageReceived(event) {
      ...
      if (now - lastBitrateMeasureTime >= 1000) {
        var bitrate = (receivedPayloadBytes - lastReceivedPayloadBytes) / (now - lastBitrateMeasureTime);
...
},{"../TestSuite":2,"../WebrtcCall":4,"jQuery":12}],9:[function(require,module,exports){
'use strict';
...
var VideoBandwidthTest = function (_Test) {
  ...
  function VideoBandwidthTest() {
    ...
    _this.constraints = {
      audio: false,
      video: {
        deviceId: _this.options.mediaOptions.video.deviceId,
        width: {
          min: 640,
          ideal: 1280,
          max: 1920
    ...
    value: function gatherStats() {
      ...
      if (now - this.startTime > this.durationMs) {
        ...
        this.call.pc1.getStats(this.localStream).then(_.bind(this.gotStats, this)).catch(function (error) {
          _this3.addLog('error', 'Failed to getStats: ' + error);
        });
      }
    }
  }, {
      ...
      if (webrtcsupport.prefix === 'webkit') {
        _.forEach(response.result(), function (report) {
          if (report.id === 'bweforvideo') {
            _this4.bweStats.add(Date.parse(report.timestamp), parseInt(report.stat('googAvailableSendBandwidth'), 10));
          } else if (report.type === 'ssrc') {
            _this4.rttStats.add(Date.parse(report.timestamp), parseInt(report.stat('googRtt'), 10));
            // Grab the last stats.
            _this4.videoStats[0] = report.stat('googFrameWidthSent');
            _this4.videoStats[1] = report.stat('googFrameHeightSent');
            _this4.packetsLost = report.stat('packetsLost');
          }
        });
...
},{"../TestSuite":2,"../WebrtcCall":4,"jQuery":12}],10:[function(require,module,exports){
'use strict';
...
},{"../TestSuite":2}],11:[function(require,module,exports){
'use strict';
